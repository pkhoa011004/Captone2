...
const createAdminAccount = async () => {
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      const insertValues = ['?', '?', '?']
      const insertParams = [email, passwordHash, 'Admin']
      ...
      if (usersColumns.has('role')) {
        ...
        insertValues.push('?')
        insertParams.push('admin')
      }

      if (usersColumns.has('role_id') && adminRoleId !== null) {
        insertColumns.push('role_id')
        insertValues.push('?')
        insertParams.push(adminRoleId)
      }
      ...
      if (usersColumns.has('role')) {
        updateParts.push("role = 'admin'")
      }

      if (usersColumns.has('role_id') && adminRoleId !== null) {
        updateParts.push('role_id = ?')
        updateParams.push(adminRoleId)
      }
      ...
      if (updateParts.length > 0) {
        ...
        console.log('Existing account has been synced to admin role')
      } else {
        console.log('Admin account exists, but no compatible role columns found to sync')
      }
